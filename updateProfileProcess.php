...
if (isset($_SESSION["u"])) {
    ...
    if (isset($_FILES["img"])) {
        ...
        if (!in_array($file_ex, $allowed_image_extentions)) {
            ...
            $file_name = "res//" . uniqid() . $new_image_extention;

            move_uploaded_file($image["tmp_name"], $file_name);

            $profile_pic_rs = Database::search("SELECT  * FROM `profile_img` WHERE `user_email` ='" . $_SESSION["u"]["email"] . "'");

            $profile_pic_num = $profile_pic_rs->num_rows;

            if ($profile_pic_num == 1) {

                Database::iud("UPDATE `profile_img` SET `path` = '" . $file_name . "' WHERE `user_email`='" . $_SESSION["u"]["email"] . "'");

                // echo "Profile Image updated Successfully";
            } else {

                Database::iud("INSERT INTO `profile_img` (`path`,`user_email`) VALUES ('" . $file_name . "','" . $_SESSION["u"]["email"] . "')");
    ...
    Database::iud("UPDATE `user` SET `first_name`='" . $fname . "' , `last_name`='" . $lname . "' , `mobile`='" . $mobile . "' WHERE `email`='" . $_SESSION["u"]["email"] . "'");
